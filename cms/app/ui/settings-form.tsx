...
export function SettingsForm({
  ...
  return (
    <div className="space-y-4">
      <form action={saveAction} className="rounded-md border border-zinc-200 bg-white p-5">
        {saved?.error ? <p className="mb-3 text-sm text-red-600">{saved.error}</p> : null}
        {saved?.message ? <p className="mb-3 text-sm text-green-700">{saved.message}</p> : null}
        ...
        <p className="mb-4 text-xs text-zinc-500">연결한 채널은 검수 화면에서 체크해 동시에 발행할 수 있습니다. 워드프레스는 기본 발행처입니다.</p>

        <section className="mb-4 rounded-md border border-zinc-200 p-3">
        ...
        <section className="mb-4 rounded-md border border-zinc-200 p-3">
          ...
          <p className="mt-2 text-xs text-zinc-500">티스토리 오픈 API access_token을 붙여 넣습니다.</p>
        </section>
        ...
        <section className="mb-4 rounded-md border border-zinc-200 p-3">
          ...
          <p className="mt-2 text-xs text-zinc-500">네이버 로그인 API 블로그 글쓰기 권한 토큰을 붙여 넣습니다.</p>
        </section>

        <SubmitButton>저장</SubmitButton>
      </form>
      <form action={pingAct} className="rounded-md border border-zinc-200 bg-white p-5">
        {ping?.error ? <p className="mb-3 text-sm text-red-600">{ping.error}</p> : null}
        {ping?.message ? <p className="mb-3 text-sm text-green-700">{ping.message}</p> : null}
